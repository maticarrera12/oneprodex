-- Include knockout_score_picks.points in the group leaderboard total.
-- Without this, RESULT points earned on the real bracket are invisible in rankings.
...
AS $function$
  SELECT u.id, u.handle, u.display_name, u.avatar_url,
         COALESCE(pred.prediction_pts, 0)
           ...
           + COALESCE(ksp.ksp_pts, 0)
           + COALESCE(u.achievement_points, 0) AS total_pts,
         COALESCE(pred.correct_count, 0)
           + COALESCE(bracket.bracket_hits, 0)
           + COALESCE(ksp.ksp_hits, 0) AS correct_count
  ...
  LEFT JOIN LATERAL (
    SELECT COALESCE(SUM(k.points), 0) AS ksp_pts,
           COUNT(CASE WHEN k.points > 0 THEN 1 END) AS ksp_hits
    FROM knockout_score_picks k
    WHERE k.user_id = gm.user_id
  ) ksp ON true
