...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { data: teamsData } = await supabase.from('teams').select('api_id, code').not('api_id', 'is', null)
    const teamCodeMap = new Map<number, string>(
      (teamsData ?? []).filter(t => t.api_id !== null).map(t => [t.api_id as number, t.code])
    )

    const { data } = await fetchFixtures(WORLD_CUP_LEAGUE_ID, WORLD_CUP_SEASON)
    const rows = data.response.map((fixture) => mapFixture(fixture, teamCodeMap))
